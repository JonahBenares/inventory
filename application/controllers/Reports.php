...
class Reports extends CI_Controller {
  ...
  function __construct(){
    ...
    public function aging_report(){
        ...
        $data['info']=array();
        if((empty($days) || $days==0) && (empty($department_id) || $department_id=='null')){
   ...
   // }
    ...
    public function export_foraccount_mothly(){
        ...
        foreach($this->super_model->custom_query("SELECT * FROM items ".$query." ORDER BY item_name ASC") AS $itm){
            ...
            $objPHPExcel->getActiveSheet()->getStyle('D'.$num)->getAlignment()->setWrapText(true);
            $count_letters=strlen($item);
            if($count_letters<=40){
...
/*
    ...
    public function issued_report(){
        ...
        $query=substr($sql,0,-3);
        $count=$this->super_model->custom_query("SELECT ih.* FROM issuance_head ih INNER JOIN issuance_details id ON ih.issuance_id = id.issuance_id INNER JOIN items i ON id.item_id = i.item_id WHERE ih.saved='1' AND ".$query. " ORDER BY ih.mif_no DESC");
       ...
       $wh_cost=array();
        if($count!=0){
           ...
           $pr_wo_cost=0;
            ...
            foreach($this->super_model->custom_query("SELECT ih.*,i.item_id, id.supplier_id, dt.department_id,pr.purpose_id,e.enduse_id, id.is_id, id.rq_id, id.brand_id, id.catalog_no, id.supplier_id, ih.pr_no, id.remarks FROM issuance_head ih INNER JOIN issuance_details id ON ih.issuance_id = id.issuance_id INNER JOIN items i ON id.item_id = i.item_id INNER JOIN department dt ON dt.department_id = ih.department_id INNER JOIN purpose pr ON pr.purpose_id = ih.purpose_id INNER JOIN enduse e ON e.enduse_id = ih.enduse_id WHERE ih.saved='1' AND ih.issuance_id = id.issuance_id AND ".$query. " ORDER BY ih.mif_no DESC") AS $itm){
    ...
    public function stock_card_new(){
      ...
      if(empty($query2)){
        ...
        foreach($this->super_model->custom_query($que_is) AS $issue){
            ...
            $receive_id = $this->super_model->select_column_join_where_order_limit("receive_id", "receive_items","receive_details", "item_id='$issue->item_id' AND pr_no='$issue->pr_no' AND supplier_id='$issue->supplier_id'","rd_id","DESC","1");
            $po_no = $this->super_model->select_column_where("receive_head", "po_no","receive_id", $receive_id);
        ...
        if(empty($query3)){
         ...
         foreach($this->super_model->custom_query($que_rs) AS $restock){
            ...
            $receive_id = $this->super_model->select_column_join_where_order_limit("receive_id", "receive_items","receive_details", "item_id='$restock->item_id' AND pr_no='$restock->from_pr' AND supplier_id='$restock->supplier_id'","rd_id","DESC","1");
            $po_no = $this->super_model->select_column_where("receive_head", "po_no","receive_id", $receive_id);
    ...
    public function export(){
        ...
        foreach($this->super_model->custom_query("SELECT rh.*,i.item_id  FROM receive_head rh INNER JOIN receive_items ri ON rh.receive_id = ri.receive_id INNER JOIN items i ON ri.item_id = i.item_id WHERE rh.saved='1' AND ".$query." GROUP BY item_name ORDER BY i.item_name ASC") AS $head){
            ...
            $objPHPExcel->getActiveSheet()->getStyle('E'.$num)->getAlignment()->setWrapText(true);
            $count_letters=strlen($item);
            if($count_letters<=40){
                $objPHPExcel->getActiveSheet()->getRowDimension($num)->setRowHeight(15);
            }else if($count_letters<=50){
        ...
        /*$objPHPExcel->getActiveSheet()->getStyle('H'L$num.":K".$num)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);*/
    ...
    public function export_restock(){
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AH10', "Remarks");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C1', "CENTRAL NEGROS POWER RELIABILITY, INC.");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C2', "Purok San Jose, Brgy. Calumangan, Bago City");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C3', "Tel. No. 476 - 7382");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C5', "FROM");
       ...
       $itemname=$this->super_model->select_column_where("items", "item_name", "item_id", $item);
        ...
        $x = 1;
            ...
            foreach($this->super_model->custom_query("SELECT rh.*,i.item_id, rd.item_cost, sr.supplier_id, rd.rdetails_id,rd.reason,rd.remarks FROM restock_head rh INNER JOIN restock_details rd ON rh.rhead_id = rd.rhead_id INNER JOIN items i ON rd.item_id = i.item_id INNER JOIN supplier sr ON sr.supplier_id = rd.supplier_id WHERE rh.saved='1' AND rh.excess='0' AND ".$query."ORDER BY rh.restock_date DESC") AS $itm){
                $supplier = $this->super_model->select_column_where('supplier', 'supplier_name', 'supplier_id', $itm->supplier_id);
                $qty = $this->super_model->select_column_where('restock_details', 'quantity', 'rdetails_id', $itm->rdetails_id); 
                $pn = $this->super_model->select_column_where('items', 'original_pn', 'item_id', $itm->item_id);
                $pr = $this->super_model->select_column_where('restock_head', 'from_pr', 'rhead_id', $itm->rhead_id);
                ...
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AH'.$num, $itm->remarks);

                // $objPHPExcel->getActiveSheet()->getProtection()->setSheet(true);    
                // $objPHPExcel->getActiveSheet()->protectCells('A'.$num.":AG".$num,'admin');
                $objPHPExcel->getActiveSheet()->getStyle('J'.$num.':AH'.$num)->getAlignment()->setWrapText(true);
                $count_letters=strlen($item);
                $count_letters_purpose=strlen($purpose);
                if($count_letters<=40){
                    $objPHPExcel->getActiveSheet()->getRowDimension($num)->setRowHeight(15);
                }else if($count_letters<=50){
                    $objPHPExcel->getActiveSheet()->getRowDimension($num)->setRowHeight(30);
                ...
                if($count_letters<=40 && $count_letters_purpose<=30){
                    $objPHPExcel->getActiveSheet()->getRowDimension($num)->setRowHeight(15);
                }else if($count_letters<=40 && $count_letters_purpose<=50){
                    $objPHPExcel->getActiveSheet()->getRowDimension($num)->setRowHeight(30);
                }else if($count_letters<=40 && $count_letters_purpose<=100){
                    $objPHPExcel->getActiveSheet()->getRowDimension($num)->setRowHeight(50);
                }else if($count_letters<=40 && $count_letters_purpose<=140){
                    $objPHPExcel->getActiveSheet()->getRowDimension($num)->setRowHeight(60);
                }else if($count_letters<=40 && $count_letters_purpose<=260){
                    $objPHPExcel->getActiveSheet()->getRowDimension($num)->setRowHeight(105);
                }
                ...
                $objPHPExcel->getActiveSheet()->getStyle($objPHPExcel->getActiveSheet()->calculateWorksheetDimension())->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);
            }
            $a = $num+2;
            $b = $num+5;
            $c = $num+4;
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A10:AH10')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    
        $objPHPExcel->getActiveSheet()->getStyle('A10:Ah'.$num)->applyFromArray($styleArray);
        $objPHPExcel->getActiveSheet()->getStyle('A3:AH3')->getBorders()->getBottom()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objPHPExcel->getActiveSheet()->getStyle('A1:AH1')->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objPHPExcel->getActiveSheet()->getStyle('A1:AH1')->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objPHPExcel->getActiveSheet()->getStyle('A2:AH2')->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objPHPExcel->getActiveSheet()->getStyle('A3:AH3')->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objPHPExcel->getActiveSheet()->getStyle('A1:AH1')->getBorders()->getRight()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objPHPExcel->getActiveSheet()->getStyle('A2:AH2')->getBorders()->getRight()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objPHPExcel->getActiveSheet()->getStyle('A3:AH3')->getBorders()->getRight()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objPHPExcel->getActiveSheet()->getStyle('D5:E5')->getBorders()->getBottom()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objPHPExcel->getActiveSheet()->getStyle('H5:I5')->getBorders()->getBottom()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objPHPExcel->getActiveSheet()->getStyle('H8:J8')->getBorders()->getBottom()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        ...
        $objPHPExcel->getActiveSheet()->getStyle('H2')->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objPHPExcel->getActiveSheet()->getStyle('H3')->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objPHPExcel->getActiveSheet()->getStyle('AH1')->getBorders()->getRight()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objPHPExcel->getActiveSheet()->getStyle('AH2')->getBorders()->getRight()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objPHPExcel->getActiveSheet()->getStyle('AH3')->getBorders()->getRight()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objPHPExcel->getActiveSheet()->getStyle('A1:D1')->getFont()->setBold(true);
        $objPHPExcel->getActiveSheet()->getStyle('H1')->getFont()->setBold(true);
        $objPHPExcel->getActiveSheet()->getStyle('C5')->getFont()->setBold(true);
        $objPHPExcel->getActiveSheet()->getStyle('G5')->getFont()->setBold(true);
       ...
       $itemname=$this->super_model->select_column_where("items", "item_name", "item_id", $item);
        ...
        $x = 1;
            ...
            foreach($this->super_model->custom_query("SELECT rh.*,i.item_id,rd.item_cost, sr.supplier_id, rd.rdetails_id, rd.reason FROM restock_head rh INNER JOIN restock_details rd ON rh.rhead_id = rd.rhead_id INNER JOIN items i ON rd.item_id = i.item_id INNER JOIN supplier sr ON sr.supplier_id = rd.supplier_id WHERE rh.saved='1' AND rh.excess='1' AND ".$query."ORDER BY rh.restock_date DESC") AS $itm){
                ...
                // $objPHPExcel->getActiveSheet()->getProtection()->setSheet(true);    
                // $objPHPExcel->getActiveSheet()->protectCells('A'.$num.":AG".$num,'admin');
                $objPHPExcel->getActiveSheet()->getStyle('J'.$num.':AG'.$num)->getAlignment()->setWrapText(true);
                $count_letters=strlen($item);
                $count_letters_purpose=strlen($purpose);
                if($count_letters<=40){
                    $objPHPExcel->getActiveSheet()->getRowDimension($num)->setRowHeight(15);
    ...
    public function export_rec(){
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('R10', "Unit Cost");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('S10', "Total Cost");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('T10', "Vat");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('U10', "Non-VAT");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('V10', "Supplier");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('Z10', "Department");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AB10', "Purpose");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AE10', "End Use");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C1', "CENTRAL NEGROS POWER RELIABILITY, INC.");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C2', "Purok San Jose, Brgy. Calumangan, Bago City");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C3', "Tel. No. 476 - 7382");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C5', "FROM");
        ...
        foreach($this->super_model->custom_query("SELECT rh.*,i.item_id, sr.supplier_id,dt.department_id,pr.purpose_id,e.enduse_id, ri.ri_id, rd.rd_id,ri.item_cost,ri.vat_status,rh.po_no,rd.pr_no FROM receive_head rh INNER JOIN receive_items ri ON rh.receive_id = ri.receive_id INNER JOIN receive_details rd ON rd.receive_id = ri.receive_id INNER JOIN items i ON ri.item_id = i.item_id INNER JOIN supplier sr ON sr.supplier_id = ri.supplier_id INNER JOIN department dt ON dt.department_id = rd.department_id INNER JOIN purpose pr ON pr.purpose_id = rd.purpose_id INNER JOIN enduse e ON e.enduse_id = rd.enduse_id WHERE rh.saved='1' AND ri.rd_id = rd.rd_id AND ".$query."ORDER BY rh.mrecf_no DESC") AS $itm) {
            $supplier = $this->super_model->select_column_where('supplier', 'supplier_name', 'supplier_id', $itm->supplier_id);
            $recqty = $this->super_model->select_column_where('receive_items', 'received_qty', 'ri_id', $itm->ri_id); 
            $pn = $this->super_model->select_column_where('items', 'original_pn', 'item_id', $itm->item_id);
            $item = $this->super_model->select_column_where('items', 'item_name', 'item_id', $itm->item_id);
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('S'.$num, $total_cost); 
            if($itm->vat_status=='vat'){
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('T'.$num, $net_of_vat);
            }else{
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('T'.$num, '');
            }
             if($itm->vat_status=='non-vat' || $itm->vat_status==''){
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('U'.$num, $total_cost);
            }else{
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('U'.$num, '');
            }
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('V'.$num, $supplier); 
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('Z'.$num, $department); 
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AB'.$num, $purpose);
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AE'.$num, $enduse);

            // $objPHPExcel->getActiveSheet()->getProtection()->setSheet(true);    
            // $objPHPExcel->getActiveSheet()->protectCells('A'.$num.":AE".$num,'admin');
            
            $objPHPExcel->getActiveSheet()->getStyle('L'.$num.':AE'.$num)->getAlignment()->setWrapText(true);
            $count_letters=strlen($item);
            $count_letters_purpose=strlen($purpose);
            if($count_letters<=40){
                $objPHPExcel->getActiveSheet()->getRowDimension($num)->setRowHeight(15);
            ...
            $objPHPExcel->getActiveSheet()->getProtection()->setFormatCells(true);
            $objPHPExcel->getActiveSheet()->getProtection()->setPassword('Inventory2024!');
            $objPHPExcel->getActiveSheet()->getStyle('A'.$num.":AG".$num)->applyFromArray($styleArray);
            $objPHPExcel->getActiveSheet()->getStyle('R'.$num.":U".$num)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
            ...
            $objPHPExcel->getActiveSheet()->mergeCells('V11:Y11');
            $objPHPExcel->getActiveSheet()->mergeCells('V'.$num.":Y".$num);
            $objPHPExcel->getActiveSheet()->mergeCells('Z11:AA11');
            $objPHPExcel->getActiveSheet()->mergeCells('Z'.$num.":AA".$num);
            $objPHPExcel->getActiveSheet()->mergeCells('AB11:AD11');
            $objPHPExcel->getActiveSheet()->mergeCells('AB'.$num.":AD".$num);
            $objPHPExcel->getActiveSheet()->mergeCells('AE11:AG11');
            $objPHPExcel->getActiveSheet()->mergeCells('AE'.$num.":AG".$num);
            ...
            $objPHPExcel->getActiveSheet()->getStyle('P11:U11')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
            $objPHPExcel->getActiveSheet()->getStyle('P'.$num.":U".$num)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
            $num++;
        }
        $a = $num+2;
        $b = $num+5;
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('G'.$a, "Approved By: ");
        $objPHPExcel->getActiveSheet()->setCellValue('G'.$b, "Plant Director/Plant Manager ");
        $objPHPExcel->getActiveSheet()->protectCells('A'.$a.":AE".$a,'admin');
        $objPHPExcel->getActiveSheet()->protectCells('A'.$c.":AE".$c,'admin');
        ...
        $objPHPExcel->getActiveSheet()->mergeCells('V10:Y10');
        $objPHPExcel->getActiveSheet()->mergeCells('Z10:AA10');
        $objPHPExcel->getActiveSheet()->mergeCells('AB10:AD10');
        $objPHPExcel->getActiveSheet()->mergeCells('AE10:AG10');
        $objPHPExcel->getActiveSheet()->mergeCells('B11:C11');
        /*$objPHPExcel->getActiveSheet()->mergeCells('B'.$num.":C".$num);
        $objPHPExcel->getActiveSheet()->mergeCells('D11:E11');
        $objPHPExcel->getActiveSheet()->mergeCells('D'.$num.":E".$num);
       ...
       /* $objPHPExcel->getActiveSheet()->getStyle('F11:G11')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);*/
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A10:AG10')->applyFromArray($styleArray);
        $objPHPExcel->getActiveSheet()->getStyle('A3:AG3')->getBorders()->getBottom()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objPHPExcel->getActiveSheet()->getStyle('A1:AG1')->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objPHPExcel->getActiveSheet()->getStyle('A1:AG1')->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $objPHPExcel->getActiveSheet()->getStyle('A2:AG2')->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
    ...
    public function export_issue(){
        ...
        $purpose     = $this->uri->segment(9);
        $pr_no       = $this->uri->segment(10);
        $department  = $this->uri->segment(11);
        $sql = '';
        ...
        if($department!='null'){
            $sql .= " ih.department_id = '$department' AND";
        }
        // $query=substr($sql,0,-3);
        $query = trim(rtrim($sql, 'AND')) ?: '1=1';
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D10', "PO No.");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F10', "MIF No.");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H10', "PR No.");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('J10', "Item Part No.");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('L10', "Item Description");
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('T10', "Unit Cost");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('U10', "Total Cost");
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('V10', "VAT");
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AT10', "Frequency");
            ...
            $wh_wo_cost=0;
            $pr_wo_cost=0;
            foreach($this->super_model->custom_query("SELECT ih.*,i.item_id, id.supplier_id, dt.department_id,pr.purpose_id,e.enduse_id, id.is_id,id.rq_id, id.remarks FROM issuance_head ih INNER JOIN issuance_details id ON ih.issuance_id = id.issuance_id INNER JOIN items i ON id.item_id = i.item_id INNER JOIN department dt ON dt.department_id = ih.department_id INNER JOIN purpose pr ON pr.purpose_id = ih.purpose_id INNER JOIN enduse e ON e.enduse_id = ih.enduse_id WHERE ih.saved='1' AND ih.issuance_id = id.issuance_id AND ".$query." ORDER BY ih.mif_no DESC") AS $itm){
                $supplier = $this->super_model->select_column_where('supplier', 'supplier_name', 'supplier_id', $itm->supplier_id);
                $issqty = $this->super_model->select_column_where('issuance_details', 'quantity', 'is_id', $itm->is_id); 
                $pn = $this->super_model->select_column_where('items', 'original_pn', 'item_id', $itm->item_id);
                $item = $this->super_model->select_column_where('items', 'item_name', 'item_id', $itm->item_id);
                ...
                $mif = $this->super_model->select_column_where('issuance_head', 'mif_no', 'issuance_id', $itm->issuance_id);
                        ...
                        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D'.$num, $po_no);
                        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F'.$num, $mif);
                        ...
                        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AO'.$num, $itm->remarks);
           ...
           foreach($item AS $i){
              $a=1;
               ...
               // echo $item . " - " . $qty . " - " . $unit_x .'<br>';
                ...
                $count_letters=strlen($item);
                $count_letters_supplier=strlen($supplier);
                if($count_letters<=40){
                    $objPHPExcel->getActiveSheet()->getRowDimension($num)->setRowHeight(15);
                }else if($count_letters<=50){
                    $objPHPExcel->getActiveSheet()->getRowDimension($num)->setRowHeight(30);
                ...
                if($count_letters<=40 && $count_letters_supplier<=30){
                    $objPHPExcel->getActiveSheet()->getRowDimension($num)->setRowHeight(15);
                }else if($count_letters<=40 && $count_letters_supplier<=50){
                    $objPHPExcel->getActiveSheet()->getRowDimension($num)->setRowHeight(30);
                }else if($count_letters<=40 && $count_letters_supplier<=100){
                    $objPHPExcel->getActiveSheet()->getRowDimension($num)->setRowHeight(50);
                }else if($count_letters<=40 && $count_letters_supplier<=140){
                    $objPHPExcel->getActiveSheet()->getRowDimension($num)->setRowHeight(50);
                }else if($count_letters<=40 && $count_letters_supplier<=260){
                    $objPHPExcel->getActiveSheet()->getRowDimension($num)->setRowHeight(105);
                }
                $objPHPExcel->getActiveSheet()->getProtection()->setSheet(true);
                $objPHPExcel->getActiveSheet()->getProtection()->setSort(true);
                ...
                if($days!='361'){
                    ...
                    if($diff>=$start_diff && $diff<=$days){
                        if($qty!=0){
                            ...
                            $objPHPExcel->getActiveSheet()->getStyle('A'.$num.':C'.$num)->getAlignment()->setWrapText(true);
                            $count_letters=strlen($item);
                            $count_letters_supplier=strlen($supplier);
                            if($count_letters<=40){
                                $objPHPExcel->getActiveSheet()->getRowDimension($num)->setRowHeight(15);
